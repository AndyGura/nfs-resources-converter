...
class NfsuSubContainer(DeclarativeCompoundBlock):
    class Fields(DeclarativeCompoundBlock.Fields):
        chunk_id = IntegerBlock(length=4)        # 0x80134001
...
class NfsuMeshContainer(DeclarativeCompoundBlock):
    class Fields(DeclarativeCompoundBlock.Fields):
        ...
        chunk_length = IntegerBlock(length=4)    # 15108
        ...
        mesh_properties = NfsuMeshPropertiesBlock()
        stream_definition = NfsuStreamDefinitionBlock() # 1536 bytes

        # 7. Stream Allocation Block (Starts with 0x00134019)
        stream_allocation = NfsuStreamAllocationBlock()
        ...
        container_remaining = BytesBlock(length=lambda ctx: (
                ...
                - 8 - ctx.data('mesh_properties/chunk_length')
                - 8 - ctx.data('stream_definition/chunk_length')
                - 8 - ctx.data('stream_allocation/chunk_length')
        ))

class NfsuBinGeometry(DeclarativeCompoundBlock):
    class Fields(DeclarativeCompoundBlock.Fields):
        header = IntegerBlock(length=4)          # 0x80134000
        file_length = IntegerBlock(length=4)     # 5761160

        # Initial 8 bytes padding gap we found earlier
        padding_gap = BytesBlock(length=8)

        # 1. The first sub-container we completely mapped out
        ...
        mesh_chunks = ArrayBlock(length=7,
                                 child=CompoundBlock(
