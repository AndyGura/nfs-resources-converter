...
class Bitmap4Bit(AnyBitmapResource, CompoundBlock):
    ...
    def __init__(self, **kwargs):
        ...
        self.block_description = 'Grayscale image, 4 bits per pixel. Used in FFN font files. In some of NFS2SE SHPI ' \
    ...
    class Fields(CompoundBlock.Fields):
        ...
        bitmap = ArrayField(length_label='width * height / 2', child=IntegerField(static_size=1, is_signed=False, ),
...
class Bitmap8Bit(AnyBitmapResource, CompoundBlock):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.block_description = '8bit bitmap can be serialized to image only with palette. Basically, for every pixel it uses ' \
