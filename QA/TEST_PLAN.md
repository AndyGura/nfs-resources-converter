...
| C-3 | Convert a small directory | Point Input at `test/golden_corpus`, Output at an empty temp dir, click Convert Files | Output directory populated with converted files per format; success snackbar; output dir opens | P0 | ✅ | **Pass** (Confirmed live, 2026-09-06, after KI-1 fix) — progress bar advanced live to "31 / 32 files processed", no `AttributeError` in the backend log. See `KNOWN_ISSUES.md` KI-1 (Fixed). Not yet re-verified against a packaged/production build. |
| C-4 | Convert with Blender/FFmpeg unset | Same as C-3 in an environment with neither tool configured | Non-3D/audio/video files still convert; skipped items land in a `skipped.txt`, no crash | P1 | ✅ | **Pass** (Confirmed live, 2026-09-06) — ran alongside C-3 (ffmpeg/Blender both absent in the test container); `skipped.txt` appeared in output, no crash. |
